...
import { Form, Input, InputNumber, Select, Rate, Switch, Slider, Radio, Checkbox, DatePicker, TimePicker, Tree, Upload, Button, Space, Tooltip } from 'antd';
  ...
  const getItem = (it) => {
    ...
    if ((it.type === 'input') && (it.field === 'item')) {
      return (
        <Space>
          ...
          <a onClick={addFormFields}><Tooltip title="增加选项"><PlusOutlined style={{ marginLeft: 5 }} /></Tooltip></a>
        </Space>
      );
    }
    ...
    if ((it.type === 'input') && (it.field.indexOf('option') != -1)) {
      return (
        <Space>
          <Input {...it} key={it.field} defaultValue={it.initialValue} placeholder={it.placeholder} style={it.style} onPressEnter={(e) => it.onPressEnter && it.onPressEnter(e.currentTarget.value)} disabled={it.disabled || false} onChange={(e) => it.onChange && it.onChange(e.target.value)} />
          <a onClick={subFormFields}><Tooltip title="删除选项"><MinusOutlined style={{ marginLeft: 5 }} /></Tooltip></a>
        </Space>
      );
    }
    if (it.type === 'input') {
