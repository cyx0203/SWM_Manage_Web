...
const His = () => {
  ...
  const [hisTrans, setHisTrans] = useState(null);
  const [statusVisible, setStatusVisible] = useState(false);
  const [hospitals, setHospitals] = useState(null);
  const args: any = useRef({ date: [moment(), moment()] });
  const TopRef = useRef(null);

  const queryHisTrans = (params) => {
    const query = { ...args.current, ...params };
    Ajax.Post('PayUrl', '/manage/chkBizTrans.selectAll',
      {
  ...
  const exportExcel = (params = {}) => {
    const query = { ...args.current, ...params };
    Ajax.Post('PayUrl', '/downloadHisDetail',
      {
        startDate: query.date && moment(query.date[0]).format('YYYYMMDD'),
        endDate: query.date && moment(query.date[1]).format('YYYYMMDD'),
        account: localStorage.getItem('account'),
  ...
  const columns: any = [
    ...
    },
    {
      ...
      dataIndex: 'operId',
      width: 150,
      render: (value, row: any) => <span>{value} {row.operName ? '('+row.operName+')' : ''}</span>
    },
    {
      title: '交易时间',
    ...
    <DomRoot>
      ...
      <Card>
        <SmartTable
          ...
          dataSource={hisTrans || []}
          columns={columns}
          // handleChange={(params: any) => queryHisTrans(params)}
