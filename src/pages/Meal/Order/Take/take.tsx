...
const Take = () => {
  ...
  const queryOrders = (params) => {
    ...
    query.hospBranchIdList = _.castArray(query.hospBranchIdList);
    
    Ajax.Post('MealUrl', '/manage/mealorderdish.take-total',
      {
        ...query,
        status:'1',
  ...
  const downloadExcel = (params) => {
    ...
    query.hospBranchIdList = _.castArray(query.hospBranchIdList);
    const bane = query?.hospBranchIdList.map((item)=>{
      return getPath(item).join("/");
    });
   
    Ajax.Post('MealUrl', '/downloadTake',
      {
        ...query,
        status:'1',
